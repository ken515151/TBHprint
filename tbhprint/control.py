...
def default_address() -> str | tuple[str, int]:
    return WINDOWS_ADDRESS if sys.platform.startswith("win") else UNIX_SOCKET_PATH
...
class Dispatcher:
    ...
    def _cmd_history(self, req):
        kwargs = {k: req[k] for k in ("status", "document_type", "since", "search") if req.get(k) is not None}
...
class ControlServer:
    def __init__(self, dispatcher: Dispatcher, address: str | tuple[str, int] | None = None,
...
class ControlClient:
    def __init__(self, address: str | tuple[str, int] | None = None, timeout: float = 10):
        self.address = address or default_address()
