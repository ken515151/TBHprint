"""SQLite persistence: job history, dedupe by server uuid, poll cursor.

Carried over from SyncroPrint for Linux (see NOTICE). Job rows are kept for
`retention` days (the server keeps its own 30-day history); spool files are
removed by the pipeline's retention sweep.
"""
...
class Store:
    ...
    def add_job(self, job_id: str, document_type: str, *, title: str | None = None,
        ...
        """Insert; False (and nothing changed) if the uuid was already seen - the dedupe gate."""
        now = time.time()
        with self._lock:
            try:
                self._db.execute(
    ...
    def set_status(self, job_id: str, status: str, *, error: str | None = None,
                   printer: str | None = None, backend_job_id: str | None = None,
                   spool_path: str | None = None, copies: int | None = None) -> None:
        now = time.time()
        sets = ["status = ?", "updated_at = ?"]
        args: list[Any] = [status, now]
        for column, value in (("copies", copies), ("error", error), ("printer", printer),
                              ("backend_job_id", backend_job_id), ("spool_path", spool_path)):
    ...
    def active_jobs(self) -> list[dict[str, Any]]:
        with self._lock:
            rows = self._db.execute(
                ...
                " ORDER BY received_at", ACTIVE_STATUSES).fetchall()
        return [self._row_to_dict(r) for r in rows]
    ...
    def spool_paths_older_than(self, days: float) -> list[tuple[str, str]]:
        ...
        with self._lock:
            rows = self._db.execute(
                ...
                (*TERMINAL_STATUSES, cutoff)).fetchall()
    ...
    def delete_older_than(self, days: float) -> int:
        ...
        with self._lock:
            cur = self._db.execute(
                f"DELETE FROM jobs WHERE received_at < ? AND status IN ({','.join('?' * len(TERMINAL_STATUSES))})",
