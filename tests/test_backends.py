...
from tbrprint.backends import PrintError, get_backend
from tbrprint.backends import cups, windows


def test_get_backend_by_name_and_auto():
    assert get_backend("cups") is cups
    assert get_backend("windows") is windows
    with mock.patch("tbrprint.backends.sys.platform", "win32"):
        assert get_backend("auto") is windows
    with mock.patch("tbrprint.backends.sys.platform", "linux"):
...
def test_cups_submit_parses_request_id():
    ...
    with mock.patch("tbrprint.backends.cups._run", return_value=proc) as run:
    ...
    with mock.patch("tbrprint.backends.cups._run", return_value=proc):
...
def test_windows_print_settings_and_sumatra_command(tmp_path):
    ...
    with mock.patch("tbrprint.backends.windows.find_sumatra", return_value=str(fake_exe)), \
         mock.patch("tbrprint.backends.windows.subprocess.run", return_value=completed) as run:
...
def test_windows_sumatra_failure_is_a_print_error(tmp_path):
    ...
    with mock.patch("tbrprint.backends.windows.find_sumatra", return_value="S.exe"), \
         mock.patch("tbrprint.backends.windows.subprocess.run", return_value=completed):
...
def test_windows_without_sumatra_off_windows_is_an_error():
    with mock.patch("tbrprint.backends.windows.find_sumatra", return_value=None), \
         mock.patch("tbrprint.backends.windows.sys.platform", "linux"):
